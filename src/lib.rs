...
mod tests {
    ...
    mod jiff_0_1 {
        ...
        impl FormatDateTime for Zoned {
            fn base() -> Self {
                ...
                let base = DateTime::from_parts(
                    Date::constant(2018, 3, 21),
                    Time::constant(11, 00, 00, 0),
                );
        ...
        fn regression_12() {
            ...
            let base =
                DateTime::from_parts(Date::constant(2024, 1, 1), Time::constant(12, 00, 00, 0));
    ...
    fn acceptance<Dt: FormatDateTime>() {
        ...
        fn assert<Dt: FormatDateTime>(input: &str, dialect: Dialect, expected: &str) {
            let date = match crate::parser::DateParser::new(input).parse(dialect) {
                Ok(date) => date,
                Err(e) => {
            ...
            let output = match crate::into_date_string(date.clone(), Dt::base(), dialect) {
                Ok(date) => date,
                Err(e) => {
                    panic!("unexpected error attempting to format {date:?}\n\t{e:?}")
                }
            };
